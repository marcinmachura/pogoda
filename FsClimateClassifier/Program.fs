...
let main argv =
    try
        if argv.Length < 2 then
            printfn "Usage: %s OUT_CSV N" (System.AppDomain.CurrentDomain.FriendlyName)
            printfn "Example: %s /tmp/out.csv 5" (System.AppDomain.CurrentDomain.FriendlyName)
            1
        else
            let outPath = argv.[0]
            let nStr = argv.[1]
            let mutable n = 0
            if not (System.Int32.TryParse(nStr, &n)) then
                ...
                printfn "Loading binary model from %s..." outBin
                let bm = BinaryModel.loadFromFile outBin
                printfn "Writing classifications to %s (n=%d)..." outPath n
                BatchClassifier.writeClassificationsCsv bm outPath n
                printfn "Done."
                0
    with ex ->
